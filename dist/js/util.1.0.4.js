...
util.ajaxPopulate = function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(populateConfig) {
    var listData, page, populateColumn, idColumn, idColumn2, _populateConfig$findJ, findJson, arrId, findJsonNeed, _ref2, data, dict;

    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            listData = populateConfig.listData, page = populateConfig.page, populateColumn = populateConfig.populateColumn, idColumn = populateConfig.idColumn, idColumn2 = populateConfig.idColumn2, _populateConfig$findJ = populateConfig.findJson, findJson = _populateConfig$findJ === undefined ? {} : _populateConfig$findJ;
            arrId = [];
            ...
            //变量：{填充查询条件}
            findJsonNeed = _defineProperty({}, idColumn2, {
              "$in": arrId
            });
            ...
            return axios({
              ...
              data: {
                findJson: findJsonNeed, pageSize: 999 //传递参数
              } });
...
util.setListPower = function (cfList) {
  ...
  var hasPowerAdd = lodash.get(window.rolePower, powerPath + ".add");
  //如果没有新增权限
  if (!hasPowerAdd) {
    //删除新增按钮
    util.handelItem({ items: cfList.batchBtns.addon, action: "delete", key: "eventType", prop: "add" });
  }
  var hasPowerDelete = lodash.get(window.rolePower, powerPath + ".delete");
  //如果没有删除权限
  if (!hasPowerDelete) {
    //删除单项删除按钮
  ...
  var hasPowerModify = lodash.get(window.rolePower, powerPath + ".modify");
  //如果没有修改权限
  if (!hasPowerModify) {
    //删除单选修改按钮
