...
function getAllUrlParame(_json) {
    ...
    _json = _json || {};
    _json.url = _json.url || window.location.href;
...
Vue.component('dialog-normal', {
    ...
    computed: {
        showDialogFlag: function showDialogFlag() {
            window.PUB.dialogZIndex = window.PUB.dialogZIndex || 2500;
            window.PUB.dialogZIndex++;
            this.zIndex = window.PUB.dialogZIndex;
...
if (window.Vuex) {
    //如果{Vuex}存在
    window.store = new Vuex.Store({ //定义Vuex的存储对象
        state: {
            debug: false,
    ...
    Vue.prototype.$store = window.store; //让vue实例中可访问$store
    document.onkeydown = function (e) {
        ...
        if (ctrlKey && keyCode == 68) {
            ...
            var debug = window.store.state.debug;
