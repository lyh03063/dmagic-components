    ...
    WIN.PUB.urlJS={
        ...
        md5:"//qn-static.dmagic.cn/md5.min.js",
        ezuikit:"//qn-static.dmagic.cn/ezuikit.js",
        xlsxFull:"//qn-static.dmagic.cn/xlsx.full.min.js",
    ...
    util.ajaxGroupDataSort = async function (actionType, doc,sortNumTarget) {
        let { _id } = doc;
        //***获取系统id,注意优先级
        ...
        await axios({
            ...
            data: { _systemId, _id, actionType ,sortNumTarget} //传递参数
        });
    };
    //#endregion
    ...
    //#region setWXJSSDK:设置初始化微信JSSDK函数（龙庭公众号的）
    util.setWXJSSDK = async function () {
        //如果在微信中
                    if (util.inWX()) {
                        ...
                        let { data } = await axios({
                            ...
                            url: `https://www.dmagic.cn/api_third_part/get_wx_jssdk_sign`,
                            data: objParam
                        });
                        ...
                        wx.config({
                            ...
                            jsApiList: [
                                ...
                                , "openAddress", "chooseImage", "scanQRCode"
                            ] // 必填，需要使用的JS接口列表
                        });
    ...
    //#region ajaxAddFileBaseInfo:ajax添加一个文件本部信息函数
    util.ajaxAddFileBaseInfo = async function (param) {
        let { fileSource, responseQiniu, fileUrl } = param
    
        let systemId = lodash.get(PUB, `_paramAjaxAddon._systemId`, PUB._systemId);
    
        let {
            fname: title, size: fileSize, ext: fileExt, w: imgWidth, h: imgHeight,
            bucket: fileBucket, time: uploadTime, key: fileKey
        } = responseQiniu
        let dataAdd = {
        ...
        await axios({
            ...
            data: { "_data": dataAdd, "_systemId": "sys_api", "_dataType": "file_base" }
        });
        console.log(`添加基础文件信息成功`);
    };
    //#endregion
    
                
    //#region ajaxDeleteBaseFile:ajax删除基本文件数据函数
    util.ajaxDeleteBaseFile = async function (url) {
        /*
        /info/commonDelete接口支持单个删除，而且id必须传，（这个规则不想改，改了风险太高！！！）
    ...
    util.cfList.sBtns.buildProductionJs={
              title: "生成生产文件",
              text: "生成生产文件",
              eventType: "buildProductionJs"
    }
                
    util.cfList.sBtns.buildDebugJs={
              title: "生成调试文件",
              text: "生成调试文件",
              eventType: "buildDebugJs"
            }
    ...
    util.zhihuigeng.globalData = {
       // urlBase: "http://47.107.49.84:8088/zhy",
        urlBase:"https://zhy.club/zhy",
        userInfo: null,
        versionCode: 1000000,
        arr_type_control: [7, 6, 4, 11, 12, 13, 14, 15],
        arr_type_env: [9, 160, 161, 162, 163, 164, 165, 166, 167, 168, 169, 170, 171, 172, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 265, 266],
    }
                
    //智慧耕操作系统信息
    util.zhihuigeng.sysInfo = { "model": "iPhone 6/7/8", "pixelRatio": 2, "windowWidth": 375, "windowHeight": 603, "system": "iOS 10.0.1", "language": "zh", "version": "7.0.4", "screenWidth": 375, "screenHeight": 667, "SDKVersion": "2.10.3", "brand": "devtools", "fontSizeSetting": 16, "benchmarkLevel": 1, "batteryLevel": 95, "statusBarHeight": 20, "safeArea": { "right": 375, "bottom": 667, "left": 0, "top": 20, "width": 375, "height": 647 }, "platform": "devtools", "devicePixelRatio": 2 }
                
    //函数：{获取智慧耕请求服务的公共headers函数}
    util.zhihuigeng.getCommonHeaders = function (onGetHeaders, requestHeaders) {
      let { sysInfo, globalData } = util.zhihuigeng;
      let headers = {};
      headers["os-type"] = sysInfo.platform;
      let version = sysInfo.system;
      if (version) {
        let splitStrs = version.split(" ");
        if (splitStrs && splitStrs.length > 1) {
          version = splitStrs[1];
        }
      }
      headers["os-version"] = version;
      headers["channel"] = "wechat";
      headers["network-type"] = "wifi";
    
      headers["version"] = globalData.versionCode;
      headers["device-id"] = "";
      headers["brand"] = "";
      headers["bundle-id"] = "SFarmChat"; //appid
      let userId = "";
      let longitude = globalData.longitude || "";
      let latitude = globalData.latitude || "";
      let userToken = "";
      // console.log('headers userInfo = ' + JSON.stringify(globalData.userInfo));
      if (globalData.userInfo && globalData.userInfo.user_id) {
        userId = globalData.userInfo.user_id;
        userToken = globalData.userInfo.user_token;
      } else if (requestHeaders) {
        userId = requestHeaders.user_id;
        userToken = requestHeaders.user_token;
      }
      if (!userId) {
        userId = "";
      }
      ...
      headers["sign"] = util.zhihuigeng.signParams(headers);
      return headers
    }
                
    //函数：{智慧耕md5加密函数}
    util.zhihuigeng.md5 = (param, upper) => {
      if (!param) {
        return "";
      }
      //  对密码进行加密
    ...
    //函数：{智慧耕专用的ajax函数}
    util.zhihuigeng.ajax = function (obj) {
        let { url } = obj;
        if (!url) return console.error("ajax的url不存在")
        if (!url.startsWith("http")) {//如果没有http开头
            url = `${util.zhihuigeng.globalData.urlBase}${url}`
        }
        let headers = util.zhihuigeng.getCommonHeaders()//调用：{获取智慧耕请求服务的公共headers函数}
        return axios({//请求接口
    ...
    //函数：{智慧耕拼接签名串函数}
    util.zhihuigeng.signParams = function (params) {
      if (!params) return "";
      let keys = [];
