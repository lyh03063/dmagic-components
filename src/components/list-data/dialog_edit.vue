...
export default {
  ...
  watch: {
    ...
    "cf.filterFormItems": {
      handler(fnFilter, oldVal) {
        this.initFormItems(); //调用：{初始化表单字段函数}
      },
      deep: true,
    },
    //监听
  ...
  methods: {
    //函数：{初始化表单字段函数}
    initFormItems: async function () {
      let fnFilter = this.cf.filterFormItems;
      if (fnFilter) {
        //Q1：{字段过滤器}存在
  ...
  created() {
    ...
    if (this.cf.listType == "common") {
      ...
      // cfFormModifyTemp.formItems = formItems; //表单字段--第一次先赋值
      if (cfForm) {
        //如果额外配置存在
