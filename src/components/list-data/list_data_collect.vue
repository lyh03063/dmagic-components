...
export default {
  ...
  data() {
    ...
    let cfList = lodash.cloneDeep(PUB.listCF.list_data_collect);
    Object.assign(cfList, {//合并对象
      ...
      comCard: "dm_card_data_collect",//卡片组件
      //卡片组件附加配置
    ...
    let { userId, dataType } = this;
    findJsonAddon.collectStatus = 1;//收藏状态
    if (userId) {
      findJsonAddon._userId = userId;
    }
    if (dataType) {
      findJsonAddon.dataType = dataType;
    }
